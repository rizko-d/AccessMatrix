...
- Fresh client and transport per run, no proxy, no cookie jar, no redirects, no connection reuse, no compression, and no retries. Tokens go only to the configured origin and only on their identity's requests.
